...
type InstitutionalCatalogSearchProps = {
  initialQuery: string;
  totalResults: number;
  totalItems: number;
};
...
export function InstitutionalCatalogSearch({
  initialQuery,
  totalResults,
  totalItems,
}: InstitutionalCatalogSearchProps) {
  ...
  return (
    <section className="rounded-sm border border-zinc-200 bg-white p-4 sm:p-5">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row">
        <label htmlFor="institutional-search" className="sr-only">
          Buscar conteudo institucional
        </label>
        <div className="relative flex-1">
          ...
          <input
            ...
            name="q"
            type="search"
            placeholder="Busque por tema, secao ou palavra-chave"
            value={query}
      ...
      <p className="mt-3 text-sm text-zinc-600">
        {initialQuery
          ? `${totalResults} resultado(s) para "${initialQuery}" em ${totalItems} conteudo(s) institucionais`
          : `${totalItems} conteudo(s) institucionais disponiveis`}
      </p>
    </section>
  );
}
