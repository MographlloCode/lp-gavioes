...
type InstitutionalCatalogPaginationProps = {
  currentPage: number;
  totalPages: number;
  query: string;
  tag: string;
...
function buildPageHref(page: number, query: string, tag: string) {
...
export function InstitutionalCatalogPagination({
  currentPage,
  totalPages,
  query,
  tag,
}: InstitutionalCatalogPaginationProps) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    <nav
      ...
      aria-label="Paginacao do catalogo institucional"
    >
      <Link
        href={buildPageHref(Math.max(1, currentPage - 1), query, tag)}
      ...
      {visiblePages.map((pageItem, index) => {
        ...
        return (
          <Link
            key={`page-${pageItem}`}
            href={buildPageHref(pageItem, query, tag)}
      ...
      <Link
        href={buildPageHref(Math.min(totalPages, currentPage + 1), query, tag)}
